...
import com.example.triviaquiz.util.Constant.QUESTION_DIFFICULTY_ANY
import com.example.triviaquiz.util.Constant.QUESTION_DIFFICULTY_EASY
import com.example.triviaquiz.util.Constant.QUESTION_DIFFICULTY_HARD
import com.example.triviaquiz.util.Constant.QUESTION_DIFFICULTY_MEDIUM
...
class QuestionFragment : Fragment(), View.OnClickListener {
    ...
    private var currentDifficulty = QUESTION_DIFFICULTY_EASY
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycle.coroutineScope.launch {
            questionViewModel.getQuestions().collect() {
                ...
                binding.tvQuestionCount.text = "$currentQuestion/$QUESTION_COUNT"
        ...
        binding.apply {
            ...
            btnNext.setOnClickListener { questionView ->
                if (!gameStatus) {
                    var highScore = false
                    lifecycle.coroutineScope.launch {
                        playerViewModel.getPlayerByName((activity as MainActivity).pref.username)
                            .collectLatest { player ->
                                if (player != null) {
                                    ...
                                    val user = (activity as MainActivity).pref.username
                                    Toast.makeText(
                                        context,
                    ...
                    var bundle =
                        bundleOf("score" to score, "level" to level, "highScore" to highScore)
                    questionView.findNavController()
                        .navigate(R.id.action_questionFragment_to_scoreFragment, bundle)
                    ...
                    currentDifficulty = when (currentDifficulty) {
                        QUESTION_DIFFICULTY_EASY -> QUESTION_DIFFICULTY_MEDIUM
                        QUESTION_DIFFICULTY_MEDIUM -> QUESTION_DIFFICULTY_HARD
                        QUESTION_DIFFICULTY_HARD -> QUESTION_DIFFICULTY_ANY
                        else -> {
                            QUESTION_DIFFICULTY_EASY
                        }
                    }
                    (activity as MainActivity).initiateQuiz(
                        QUESTION_COUNT,
                        currentDifficulty,
                        object :
                            QuestionCall {
                    ...
                    Toast.makeText(
                        activity,
                        "${(activity as MainActivity).pref.username} choose an answer.",
                        Toast.LENGTH_SHORT
                    ).show()
                }
            }
    ...
    override fun onClick(btn: View?) {
        ...
        if (!answerStatus) {
            ...
            if (TextUtils.equals(answer, questionList[currentQuestion - 1].correct_answer)) {
                btnMat.strokeColor = ColorStateList.valueOf(
    ...
    private fun showAnswer() {
        binding.apply {
            for (i in 0 until llAnswers.childCount) {
                ...
                if (TextUtils.equals(
                        child.text,
                        questionList[currentQuestion - 1].correct_answer.parseAsHtml()
                    )
                ) {
                    child.strokeColor = ColorStateList.valueOf(
                        ContextCompat.getColor(
                            requireContext(),
                            R.color.beige
                        )
